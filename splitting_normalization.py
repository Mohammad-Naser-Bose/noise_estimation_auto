...
def normalization (train_no_norm, val_no_norm, test_no_norm):
    ...
    scaler_standard.fit(all_windows_training.reshape(-1,1))
    normalized_training_windows = {key:scaler_standard.transform(np.array(value).reshape(-1,1)).flatten().tolist() for key, value in train_no_norm.items()}
    normalized_validation_windows = {key:scaler_standard.transform(np.array(value).reshape(-1,1)).flatten().tolist() for key, value in val_no_norm.items()}
    normalized_testing_windows = {key:scaler_standard.transform(np.array(value).reshape(-1,1)).flatten().tolist() for key, value in test_no_norm.items()}

    scaler_minmax = MinMaxScaler()
    scaler_minmax.fit(all_windows_training.reshape(-1,1))
    normalized_training_windows = {key:scaler_minmax.transform(np.array(value).reshape(-1,1)).flatten().tolist() for key, value in normalized_training_windows.items()}
    normalized_validation_windows = {key:scaler_minmax.transform(np.array(value).reshape(-1,1)).flatten().tolist() for key, value in normalized_validation_windows.items()}
    normalized_testing_windows = {key:scaler_minmax.transform(np.array(value).reshape(-1,1)).flatten().tolist() for key, value in normalized_testing_windows.items()}
...
def FE(data_1, data_2, data_3):
    ...
    temp_arr = np.hstack(list(RMS_values_1.values())).reshape(-1, 1)
    scaler_standard = StandardScaler()
    scaler_standard.fit(temp_arr)
    temp_arr_new = scaler_standard.transform(temp_arr)
    scaler_minmax = MinMaxScaler()
    scaler_minmax.fit(temp_arr_new)
    temp_arr_new = scaler_minmax.transform(temp_arr_new)
    RMS_values_new_1 = {}
    for i in range(0,len(temp_arr_new)):
        RMS_values_new_1[i] = np.array(temp_arr_new[i], dtype= np.float32)
